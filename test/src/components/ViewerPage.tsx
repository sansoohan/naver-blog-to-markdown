...
function ViewerPage({
  ...
  const [markdownZoom, setMarkdownZoom] =
    useState(() => {
      const saved = Number(
        localStorage.getItem("markdownZoom")
      );
  ...
  const getOriginalUrl = (
    ...
    return `/api/post/original?path=${encodeURIComponent(
      post.relativePath
    )}&refresh=${refreshKey}`;
  };
  ...
  const getMarkdownUrl = (
    ...
    return `/api/post/markdown?path=${encodeURIComponent(
      post.relativePath
    )}&refresh=${refreshKey}`;
  };

  const changeMarkdownZoom = (
    amount: number
  ) => {
  ...
  const resetMarkdownZoom = () => {
    setMarkdownZoom(
      DEFAULT_MARKDOWN_ZOOM
    );

    localStorage.setItem(
      "markdownZoom",
      String(DEFAULT_MARKDOWN_ZOOM)
    );
  };
