...
function isNaverPostListUnavailable(error) {
  const message=String(error?.message||error);
  return /게시글 목록 요청 실패:\s*일시적으로 목록보기 기능에 장애가 발생하였습니다/.test(message);
}
...
async function getPostListWithRetry(blogId,categoryNo,page,options={}) {
  const {includePrivate=false,maxAttempts=5,allowUnavailableCategory=false}=options;
  let lastError;
  ...
  for(let attempt=1;attempt<=maxAttempts;attempt++) {
    try {
      ...
      if(allowUnavailableCategory&&page===1&&isNaverPostListUnavailable(error)) {
        console.warn(`목록을 가져올 수 없어 건너뜁니다. (categoryNo=${categoryNo})`);
      ...
      console.warn(
        ...
        +`${waitSeconds}초 후 재시도합니다. (${attempt}/${maxAttempts}, categoryNo=${categoryNo}, page=${page})`
      );
...
async function getAllPosts(blogId,categoryNo,options={}) {
  const {quiet=false,includePrivate=false,allowUnavailableCategory=false}=options;
  const posts=[];
  const seen=new Set();
  let page=1;
  let totalCount=null;
  ...
  while(true) {
    const data=await getPostListWithRetry(blogId,categoryNo,page,{includePrivate,allowUnavailableCategory});
    const postList=Array.isArray(data.postList)?data.postList:[];

    if(totalCount===null) {
      const parsedTotalCount=Number(data.totalCount);
      totalCount=Number.isFinite(parsedTotalCount)&&parsedTotalCount>=0?parsedTotalCount:null;
    }
    ...
    if(!added) {
      if(!quiet) console.warn(`새 게시글이 없는 페이지가 나와 목록 검색을 종료합니다. (page=${page})`);
      break;
    }
...
function getDescendantLeafCategories(category,categories) {
  return getDescendantCategories(category,categories).filter(item=>!getChildren(item,categories).length);
}
...
function getDirectPosts(category,categories,postLists) {
  ...
  for(const descendant of descendants) {
    ...
    for(const post of posts) descendantPostNos.add(post.logNo);
  }

  return ownPosts.filter(post=>!descendantPostNos.has(post.logNo));
}

async function getAllCategoryPosts(blogId,categories,options={}) {
  const {includePrivate=false,quiet=false}=options;
  ...
  for(let index=0;index<categories.length;index++) {
    ...
    if(!quiet) console.log(`글 목록 확인 중: ${index+1}/${categories.length} ${pathText}`);

    const categoryPosts=await getAllPosts(blogId,category.categoryNo,{
      quiet:true,
      includePrivate,
      allowUnavailableCategory:true,
    });

    postLists.set(category.categoryNo,categoryPosts);
  }
  ...
  for(const category of categories) {
    const categoryPath=getCategoryPathParts(category,categories);
    const directPosts=getDirectPosts(category,categories,postLists);

    if(!quiet&&directPosts.length) {
...
async function getCategoryPosts(blogId,category,categories,options={}) {
  ...
  for(let index=0;index<targetCategories.length;index++) {
    const targetCategory=targetCategories[index];
